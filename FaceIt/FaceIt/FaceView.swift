...
//  FaceIt
//
//  Created by Ajit Kumar Baral on 12/30/16.
...
class FaceView: UIView {
    ...
    private func pathForMouth()->UIBezierPath{
        ...
        let smileOffsset = CGFloat(max(-1, min(mouthCurvature, 1))) * mouthRect.height
        let start = CGPoint(x: mouthRect.minX, y: mouthRect.minY)
        let end = CGPoint(x: mouthRect.maxX, y: mouthRect.minY)
        let cp1 = CGPoint(x: mouthRect.minX + mouthRect.width/3 , y: mouthRect.minY + smileOffsset)
        let cp2 = CGPoint(x: mouthRect.maxX - mouthRect.width/3 , y: mouthRect.minY + smileOffsset)
